 ...
 *
 * @package GenesisResponsiveSlider
 */
...
function register_genesis_responsive_slider_settings() {
	...
	if ( genesis_get_responsive_slider_option( 'reset' ) ) {
		...
		genesis_admin_redirect( 'genesis_responsive_slider', array(
			'reset' => 'true',
		) );
		exit;
	}
...
function genesis_responsive_slider_notice() {
	...
	if ( isset( $_REQUEST['reset'] ) && 'true' == $_REQUEST['reset'] ) {
		...
		echo '<div id="message" class="updated"><p><strong>' . esc_html__( 'Settings reset.', 'genesis-responsive-slider' ) . '</strong></p></div>';

	} elseif ( isset( $_REQUEST['settings-updated'] ) && 'true' == $_REQUEST['settings-updated'] ) {

		echo '<div id="message" class="updated"><p><strong>' . esc_html__( 'Settings saved.', 'genesis-responsive-slider' ) . '</strong></p></div>';
 ...
 *
 * @param  array $columns Slider columns.
 * @param  int   $screen  Slider screen.
 * @return array
 */
 ...
 * It handles the markup, builds the form, outputs necessary
 * JS stuff, and fires <code>do_meta_boxes()</code>.
 */
function genesis_responsive_slider_settings_admin() {
	...
	<form method="post" action="options.php">
		...
		<?php settings_fields( GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD ); ?>

		<?php screen_icon( 'plugins' ); ?>
		<h2>
			<?php esc_html_e( 'Genesis - Responsive Slider', 'genesis-responsive-slider' ); ?>
			<input type="submit" class="button-primary genesis-h2-button" value="<?php esc_attr_e( 'Save Settings', 'genesis-responsive-slider' ); ?>" />
			<input type="submit" class="button-highlighted genesis-h2-button" name="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[reset]" value="<?php esc_attr_e( 'Reset Settings', 'genesis-responsive-slider' ); ?>" onclick="return genesis_confirm( '<?php echo esc_js( __( 'Are you sure you want to reset?', 'genesis-responsive-slider' ) ); ?>' );" />
		</h2>
		...
		<div class="bottom-buttons">
			<input type="submit" class="button-primary" value="<?php esc_attr_e( 'Save Settings', 'genesis-responsive-slider' ); ?>" />
			<input type="submit" class="button-highlighted" name="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[reset]" value="<?php esc_attr_e( 'Reset Settings', 'genesis-responsive-slider' ); ?>" />
		</div>
...
function genesis_responsive_slider_options_box() {
	...
	<div id="genesis-responsive-slider-content-type">
		...
		<p><label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[post_type]"><?php esc_html_e( 'Would you like to use posts or pages', 'genesis-responsive-slider' ); ?>?</label>
			...
			foreach ( $post_types as $post_type ) {
				...
				?>
				<option style="padding-right:10px;" value="<?php echo esc_attr( $post_type ); ?>" <?php selected( esc_attr( $post_type ), genesis_get_responsive_slider_option( 'post_type' ) ); ?>><?php echo esc_attr( $post_type ); ?></option>
	...
	<div id="genesis-responsive-slider-content-filter">
		...
		<div id="genesis-responsive-slider-taxonomy">
			...
			<p><strong style="display: block; font-size: 11px; margin-top: 10px;"><?php esc_html_e( 'By Taxonomy and Terms', 'genesis-responsive-slider' ); ?></strong><label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[posts_term]"><?php esc_html_e( 'Choose a term to determine what slides to include', 'genesis-responsive-slider' ); ?>.</label>
					...
					foreach ( $taxonomies as $taxonomy ) {
						...
						?>
							<optgroup label="<?php echo esc_attr( $taxonomy->labels->name ); ?>">
								...
								<option style="margin-left: 5px; padding-right:10px;" value="<?php echo esc_attr( $query_label ); ?>" <?php selected( esc_attr( $query_label ), genesis_get_responsive_slider_option( 'posts_term' ) ); ?>><?php echo $taxonomy->labels->all_items; ?></option><?php $terms = get_terms( $taxonomy->name, 'orderby=name&hide_empty=1' );
			...
			<p>
				<label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[exclude_terms]"><?php printf( esc_html__( 'List which category, tag or other taxonomy IDs to exclude. (1,2,3,4 for example)', 'genesis-responsive-slider' ), '<br />' ); ?></label>
			</p>
		...
		<p>
			<strong style="font-size:11px;margin-top:10px;"><label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[include_exclude]"><?php /* Translators: Include or exclude by ID number */ printf( esc_html__( 'Include or Exclude by %s ID', 'genesis-responsive-slider' ), genesis_get_responsive_slider_option( 'post_type' ) ); ?></label></strong>
		</p>
		...
		<p>
			<select style="margin-top: 5px;" id="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[includeesc_htmlxclude]" name="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[include_exclude]">
		...
		<p>
			<label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[post_id]"><?php esc_html_e( 'List which', 'genesis-responsive-slider' ); ?> <strong><?php echo genesis_get_responsive_slider_option( 'post_type' ) . ' ' . esc_html__( 'ID', 'genesis-responsive-slider' ); ?>s</strong> <?php esc_html_e( 'to include / exclude. (1,2,3,4 for example)', 'genesis-responsive-slider' ); ?></label></p>
		<p>
		...
		<p>
			<label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[posts_num]"><?php esc_html_e( 'Number of Slides to Show', 'genesis-responsive-slider' ); ?>:</label>
		...
		<p>
			<label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[posts_offset]"><?php esc_html_e( 'Number of Posts to Offset', 'genesis-responsive-slider' ); ?>:</label>
		...
		<p>
			<label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[orderby]"><?php esc_html_e( 'Order By', 'genesis-responsive-slider' ); ?>:</label>
	...
	<h4><?php esc_html_e( 'Transition Settings', 'genesis-responsive-slider' ); ?></h4>
		...
		<p>
			<label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_timer]"><?php esc_html_e( 'Time Between Slides (in milliseconds)', 'genesis-responsive-slider' ); ?>:
		...
		<p>
			<label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_delay]"><?php esc_html_e( 'Slide Transition Speed (in milliseconds)', 'genesis-responsive-slider' ); ?>:
		...
		<p>
			<label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_effect]"><?php esc_html_e( 'Slider Effect', 'genesis-responsive-slider' ); ?>:
	...
	<h4><?php esc_html_e( 'Display Settings', 'genesis-responsive-slider' ); ?></h4>
		...
		<p>
			<label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_width]"><?php esc_html_e( 'Maximum Slider Width (in pixels)', 'genesis-responsive-slider' ); ?>:
		...
		<p>
			<label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_height]"><?php esc_html_e( 'Maximum Slider Height (in pixels)', 'genesis-responsive-slider' ); ?>:
		...
		<p>
			<input type="checkbox" name="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_arrows]" id="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_arrows]" value="1" <?php checked( 1, genesis_get_responsive_slider_option( 'slideshow_arrows' ) ); ?> /> <label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_arrows]"><?php esc_html_e( 'Display Next / Previous Arrows in Slider?', 'genesis-responsive-slider' ); ?></label>
		</p>
		
		<p>
			<input type="checkbox" name="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_pager]" id="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_pager]" value="1" <?php checked( 1, genesis_get_responsive_slider_option( 'slideshow_pager' ) ); ?> /> <label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_pager]"><?php esc_html_e( 'Display Pagination in Slider?', 'genesis-responsive-slider' ); ?></label>
		</p>
	...
	<h4><?php esc_html_e( 'Content Settings', 'genesis-responsive-slider' ); ?></h4>
		...
		<p>
			<input type="checkbox" name="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_excerpt_show]" id="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_excerpt_show]" value="1" <?php checked( 1, genesis_get_responsive_slider_option( 'slideshow_excerpt_show' ) ); ?> /> <label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_excerpt_show]"><?php esc_html_e( 'Display Content in Slider?', 'genesis-responsive-slider' ); ?></label>
		</p>
		
		<p>
			<input type="checkbox" name="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_hide_mobile]" id="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_hide_mobile]" value="1" <?php checked( 1, genesis_get_responsive_slider_option( 'slideshow_hide_mobile' ) ); ?> /> <label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_hide_mobile]"><?php esc_html_e( 'Hide Title & Content on Mobile Devices', 'genesis-responsive-slider' ); ?></label>
		</p>
		...
		<p>
			<label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_more_text]"><?php esc_html_e( 'More Text (if applicable)', 'genesis-responsive-slider' ); ?>:</label>
		...
		<p>
			<label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_excerpt_content_limit]"><?php esc_html_e( 'Limit content to', 'genesis-responsive-slider' ); ?></label>
			...
			<label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_excerpt_content_limit]"><?php esc_html_e( 'characters', 'genesis-responsive-slider' ); ?></label>
		</p>
		...
		<p>
			<label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[slideshow_excerpt_width]"><?php esc_html_e( 'Slider Excerpt Width (in percentage)', 'genesis-responsive-slider' ); ?>:
		...
		<p>
			<label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[location_vertical]"><?php esc_html_e( 'Excerpt Location (vertical)', 'genesis-responsive-slider' ); ?>:</label>
		...
		<p>
			<label for="<?php echo GENESIS_RESPONSIVE_SLIDER_SETTINGS_FIELD; ?>[location_horizontal]"><?php esc_html_e( 'Excerpt Location (horizontal)', 'genesis-responsive-slider' ); ?>:</label>
